 ...
 * and per-axis optMaybe the user is drawing something other than a line.
      // TODO(danvk): test this case.
      realStroke.call(this);
      return;
    }
...
/**
 * Get a value for a specific series. If there  'Y1: { axis: {} }, 'Y2': { axis: 'Y1' } }
    for (var idx = 0; idx < this.labels_.length; idx++) {
      var seriesName = this.labels_[idx];
      var optionsForSeries = this.series_[seriesName]["options"];
      var axis = optionsForSeries["axis"];
 ...
 *
 * This class contains all the data to be chis no specific value for the series,
 * the value for the axis is returned (and afterwards, the global value.)
 *
 * @param {string} name the name of the option.
 * @param {string} series the series to search.
...
var DygraphCanvasRenderer = function(dygraph, element, elementContext, layout) {
  ...
  if (this.dygraph_.isUsingExcanvas_) {
    ...
    // on Android 3 and 4, setting a clipping ah_.toPercentXCoord(tick.v);
    if ((pos >= 0.0) && (pos < 1.0)) {
      this.xticks.push([pos, label]);
    }
  }
...
DygraphCanvasRenderer._drawSeries = function(e,
 ...
 rea on a canvas prevents it from
    // displaying anything.
    if (!Dygraph.isAndroid()) {
      var ctx = this.dygraph_.canvas_ctx_;
      ctx.beginPath();
 ...
 */
DygraphCanvasRenderer._drawStyledLine = function(e,
    color,     prevCanvasY = point.canvasy;
    }
    first = false;
  }
  ctx.stroke();
  ...
  for (i = 0; i < plotters.length; i++) {
    ...
    for (var j = 0; j < sets.length; j++) {
      ...
      p({
        points: points,
        setName: setName,
        drawingContext: ct strokeWidth, strokePattern, drawPoints,
    drawPointCallback, pointSize) {
  var g = e.dygraph;
  // TODO(konigsberg): Compute attributes outside this method call.
  var stepPlot = g.getBooleanOption("stepPlot", e.setName);
  ...
  for (i = 0; i < plotters.length; i++) {
    ...
    for (var j = 0; j < sets.length; j++) {
      ...
      p({
        points: points,
        setName: setName,
        drawingContext: ct ctx.moveTo(prevX, prevYs[0]);
...
DygraphCanvasRenderer._fillPlotter = function(e) {
  ...
  for (var setIdx = setCount - 1; setIdx >= 0; setIdx--) {
    ...
    ctx.fillStyle = err_color;
    ctx.beginPath();
    var l ctx.moveTo(prevX, prevYs[0]);
        ctx.lineTo(point.canvasx, prevYs[0]);
        ctx.lineTo(point.canvasx, prevYs[1]);
      } else {
        ctx.moveTo(prevX, prevYs[0]);
...
Dygraph.numberValueFormatter = function(x, opts) {
  ...
  if (kmb || kmg2) {
    ...
    if (kmg2) {
      if (kmb) console.warn("Setting ast_x, is_first = true;
...
Dygraph.DEFAULT_ATTRS = {
  ...
  strokeWidth: 1.0,
  strokeBorderWidth: 0,
  strokboth labelsKMB and labelsKMG2. Pick one!");
      k = 1024;
      k_labels = Dygraph.KMG2_BIG_LABELS;
      m_labels = Dygraph.KMG2_SMALL_LABELS;
    }
...
Dygraph.prototype.__init__ = function(div, file, attrs) {
  ...
  if (/MSIE/.test(navigator.userAgent) && !window.opera &&eBorderColor: "white",
...
Dygraph.prototype.optionsViewForAxis_ = function(axis) {
  ...
  return function(opt) {
    ...
    if (axis === 'x' && opt === 'logscale') {
      ...
      // TODO(konigsberg): pull the default from a global d    } else {
      pluginInstance = new Plugin();
    }
...
Dygraph.prototype.toDomYCoord = function(y, axis) {
  ...
  var area = this.plotter_.area;
  return area.y + pct returned. This is optional.
 * @return { ... } The value of the option.
 */
Dygraph.prototype.attr_ = function(name, seriesName) {
  if (DEBUG) {
...
Dygraph.prototype.optionsViewForAxis_ = function(axis) {
  ...
  return function(opt) {
    ...
    if (axis === 'x' && opt === 'logscale') {
      // return the default value.
      // TODO(konigsberg): pull the default from a global d * area.h;
};
...
Dygraph.prototype.toDomYCoord = function(y, axis) {
  ...
  var area = this.plotter_.area;
  return area.y + pct} y The data y-coordinate.
 ...
 * if axis is null, this uses the first axis.
 *
 * @param {numberas_ctx_ = Dygraph.getContext(this.canvas_);
...
Dygraph.prototype.createRollInterface_ = function() {
  ...
  if (!this.roller_) {
    ...
    this.graphDiv.appenrn {Object} The newly-created canvas
 * @private
 */
Dygraph.prototype.createPlotKitCanvas_ = function(canvas) {
  var h = Dygraph.createCanvas();
 ...
 * method with doZoomX which dHandler(interactionModel[eventName]));
  }
...
Dygraph.prototype.resetZoom = function() {
  ...
  if (dirty) {
    ...
    if (dirtyY) {
      ...
      var packed = this.gatherDatasets_(this.rolledSeries_, null);accepts pixel coordinates. This function redraws
 * the graph.
 *
 * @param {number} minDate The minimum date that should be visible.
 * @param {number} maxDate The maximum date that should be visible.
...
Dygraph.prototype.mouseMove_ = function(event) {
  ...
  if (highlightSeriesOpts && !this.isSeriesLocked()) {
    ...
    if (this.getBooleanOption("stackedGraph")) {
      ...
      closest = this.findC* Given canvas X,Y coordinates, find the closest point.
 *
 * This finds the individual data point across all visible series
 * that's closest to the supplied DOM coordinates using the standard
 * Euclidean X,Y distance.
...
Dygraph.prototype.updateSelection_ = function(opt_animFraction) {
  ...
  if (this.selPoints_.length > 0) {
    ...
    ctx.save();
    for (i = 0; i < this.selPoints_.length; i++)losestPoint(canvasx, canvasy);
    }
    selectionChanged = this.setSelection(closest.row, closest.seriesName);
  } else {
    var idx = this.findClosestRow(canvasx);
...
Dygraph.stackPoints_ = function(
  ...
  for (var i = 0; i < points.length; ++i) {
    ...
    if (lastXval != xval) {
      // If an x- i = 1; i < this.numColumns(); i++) {
    // var logScale = this.attr_('logscale', i); // TODO(klausw): this looks wrong // konigsberg thinks so too.
    var series = this.dataHandler_.extractSeries(this.rawData_, i, this.attributes_);
    if (this.rollPeriod_ > 1) {
      series = this.dataHandler_.rollingAverage(series, this.rollPeriod_, this.attributes_);
...
Dygraph.prototype.gatherDatasets_ = function(rolledSeries, dateWindow) {
  ...
  for (seriesIdx = num_series; seriesIdx >= 1; seriesIdx--) {
    ...
    if (this.getBooleanOption("stackedGraph")) {
      ...
      if (cumulativeYval[axisIdxvalue is repeated, we ignore the duplicates.
      stackedYval += actualYval;
      cumulativeYval[xval] = stackedYval;
    }
    lastXval = xval;
...
Dygraph.prototype.computeYAxes_ = function() {
  ...
  if (this.axes_ !== undefined && this.user_attrs_.hasOwnProperty("value] === undefined) {
        cumulativeYval[axisIdx] = [];
      }
      Dygraph.stackPoints_(seriesPoints, cumulativeYval[axisIdx], seriesExtremes,
                           this.getBooleanOption("stackedGraphNaNFill"));
...
Dygraph.prototype.computeYAxisRanges_ = function(extremes) {
  ...
  for (var i = 0; i < numAxes; i++) {
    ...
    if (series.length === 0) {
      // If no series are defined or visible then use a reasRange") === false) {
    valueWindows = [];
    for (index = 0; index < this.axes_.length; index++) {
      valueWindows.push(this.axes_[index].valueWindow);
    }
 ...
 * are dates, we need to con if (!axis.independentTicks) {
      var opts = this.optionsViewForAxis_('y' + (i ? '2' : ''));
      var ticker = opts('ticker');
      var p_ticks = p_axis.ticks;
      var p_scale = p_axis.computedValueRange[1] - p_axis.computedValueRange[0];
...
Dygraph.prototype.parseCSV_ = function(data) {
  ...
  for (var i = start; i < lines.length; i++) {
    ...
    line_no = i;
    if (line.length === 0) continue;  // skip blank lines
    if (line[0] == '#') continue;   vert them from Date objects to ms since epoch.
 * @param {!Array} data
...
Dygraph.prototype.parseDataTable_ = function(data) {
  ...
  if (indepType == 'date' || indepType == 'datetime') {
    ...
    console.error("only 'date', 'datetime' and 'number' types are supported " +
                  "for column 1 of DataTable input (Got '" + indepType + " // skip comment lines
    var inFields = line.split(delim);
    if (inFields.length < 2) continue;
 ...
 * @license
 * Copyright 2011 Dan Vanderkam (danvdk@gmail.com)
 * MIT-licensed (htreturn A {width: w, height: h} object.
 * @private
 */
Dygraph.prototype.size = function() {
  return { width: this.width_, height: this.height_ };
 ...
 * @private
 */
Dygraph.cancelEvent = functitp://opensource.org/licenses/MIT)
 */
...
Dygraph.DateAccessorsLocal = {
  ...
  getMonth:        function(d) {return d.getMonth();p || 0);
  }
};
...
Dygraph.updateDeep = function (self, o) {
  ...
  if (typeof(o) != 'undefined' && o f (low > high) {
    return -1;
  }
  if (abs === null || abs === undefined) {
    abs = 0;
...
Dygraph.repeatAndCleanup = function(repeatFn, maxFrames, framePeriodInMillis,
  ...
  (function loop() {
    ...
    Dygraph.requestAnimFrame.call(window, function() {
      ...
      var predictOvershootStutter = (frameNumber + frameDelta) > maxFis.array_ = array;
  this.predicate_ = predicate;
  this.end_ = Math.min(array.length, start + length);
  this.nextIdx_ = start - 1; // use -1 so initial advance works.
  this.next(); // ignoring result.
...
/**
 * Converts any valid CSS color (heler on the element where the drag begins. In that handler, you register a
 * mouseup handler on the window to determine when the mouse is released,
 * wherever that release happens. This works well, except when the user releases
 * the mouse over an off-domain iframe. In that case, the mouseup event is
 * handled by the iframe and never bubbles up to the window handler.
...
/**
 * Googlx, rgb(), named color) to an RGB tuple.
 *
 * @param {!string} colorStr Any valid CSS color string.
 * @return {{r:number,g:number,b:number,a:number?}} Parsed RGB tuple.
 * @private
...
Dygraph.Interaction.startPan = function(event, g, context) {
  ...
  if (g.getNumericOption("panEdgeFraction")) {
    ...
    var boundedValuese charts compatible setSelection
 * Only row selection is supported, all points in the row will be highlighted
 * @param {Array.<{row:number}>} selection_array array of the selected cells
 * @public
 */
...
Dygraph.Interaction.treatMouseOpAsClick = function(g, event, context) {
  ...
  // Allow any click within two pixels of tute minValue.
          maxValue = maxValue - (minValue - boundedValue[0]);
          minValue = maxValue - axis_data.dragValueRange;
        }
      }
...
Dygraph.Interaction.endTouch = function(event, g, context) {
  if (event.touches.length !== 0) {
    ...
    // The heuristic here is that it's a double-tap if the two touc      dataX: 0.5 * (touches[0].dataX + touches[1].dataX),
      dataY: 0.5 * (touches[0].dataY + touches[1].dataY)
    };
...
Dygraph.TICK_PLACEMENT[Dygraph.SIX_HOURLY]      = {datefield: Dygraph.DATEFIELD_HH, step:   6,xels / nTicks;
        if (spacing > pixels_per_tick) break;
      }
...
Dygraph.Plugins.Annotations =a nice tick position before the initial instant.
  // Currently, this code deals properly with the existent daily granularities:
  // DAILY (with step of 1) and WEEKLY (with step of 7 but specially handled).
  // Other daily granularities (say TWO_DAILY) should also be handled specially
  // by setting the start_date_offset to 0.
...
annotations.prototype.didDrawChart = function(e) {
  ...
  for (var i = 0; i < points.length; i++) {
    ...
    g.addAndTrackEvent(div, 'mouseout',
        bindEvt('mouseOutHandler', 'annotationMouseOutHandl (function() {
...
axes.prototype.willDrawChart = function(e) {
  ...
  var makeLabelStyle = function(axis) {
    return {
      ...
      lineHeight: 'normal',  // Something otherer', p, this));
    g.addAndTrackEvent(div, 'dblclick',
        bindEvt('dblClickHandler', 'annotationDblClickHandler', p, this));
...
axes.prototype.willDrawChart = function(e) {
  ...
  if (g.getOptionForAxis('drawAxis', 'y')) {
    ...
    context.l than "normal" line-height screws up label positioning.
      overflow: 'hidden'
    };
  };
 ...
 * @license
 * Copyright 2012 Dan Vanderkam (danvdk@gmail.com)
 * MIT-licensed (http://opensourceineWidth = g.getOptionForAxis('axisLineWidth', 'y');
...
legend.prototype.select = function(e) {
  ...
  if (legendMode === 'follow') {
    ...
    // offset 20 px to the right and down f.org/licenses/MIT)
 */
/*global Dygraph:false */
...
legend.generateLegendHTML = function(g, x, sel_points, oneEmWidth, row) {
  ...
  if (typeof(x) === 'undefined') {
    ...
    for (i = 1; i < labels.length; i++) {
      ...
      dash = generateLegendDashHrom the first selection point
    // 20 px is guess based on mouse cursor size
    var leftLegend = points[0].x * area.w + 20;
    var topLegend  = points[0].y * area.h - 20;
...
generateLegendDashHTML = function(strokePattern, color, oneEmWidth) {
  ...
  TML(strokePattern, series.color, oneEmWidth);
      html += "<span style='font-weight: bold; color: " + series.color + ";'>" +
          dash + " " + escapeHTML(labels[i]) + "</span>";
    }
    return html;
 ...
 */
rangeSelector.prototype.renderInteractiveLayer_      // The repeated first segment has no right margin.
        marginRight = 0;
      }
      dash += "<div style=\"display: inline-block; position: relative; " +
        "bottom: .5ex; margin-right: " + marginRight + "em; padding-left: " +
...
rangeSelector.prototype.initInteraction_ = function() {
  ...
  isMouseInPanZone = function(e) {
    if (self.isUsingExcanvas_) {
      ...
      var righqSTDH32I1pQA2Pb9sZecAxc5r3IAb21d6878xsAAAAAASUVORK5CYII=';
  }
...
rangeSelector.prototype.initInteraction_ = function() {
  ...
  if (this.isUsingExcanvas_) {
    this.dygraph_.addAndTrackEvent(tHandleClientX = rect.left + rect.width/2;
      return (e.clientX > leftHandleClientX && e.clientX < rightHandleClientX);
    }
  };
...
rangeSelector.prototype.computeCombinedSeriesAndLimits_ = function() {
  ...
  var numColumns = gthis.iePanOverlay_, 'mousedown', onPanStart);
  } else {
    this.dygraph_.addAndTrackEvent(this.fgcanvas_, 'mousedown', onPanStart);
    this.dygraph_.addAndTrackEvent(this.fgcanvas_, 'mousemove', onCanvasHover);
  }
...
Dygraph.OPTIONS_REFERENCE =  // <JSON>
{
  ...
  "stackedGraphNaNFill": {
    ...
    "labels": ["Data Line display"],
    "type": "string",
    "description": "Controls handling of NaN values inside a stacked graph. NaN values are interpolated/extended for stacking purposes, but the actual point value remains NaN in the legend display. Valid option values are \"all\" (interpolay_) {
      this.iePanOverlay_.style.display = 'none';
    }
  } else {
    var leftHandleCanvasPos = Math.max(margin, zoomHandleStatus.leftHandlePos - this.canvasRect_.x);
...
Dygraph.OPTIONS_REFERENCE =  // <JSON>
{
  ...
  "colors": {
    ...
    "type": "array<string>",
    "example": "['red', '#00FF00']",
    "description": "List of colors for the data series. These can be of the form \"#AABBCC\" or \"rgb(25late internally, repeat leftmost and rightmost value as needed), \"inside\" (interpolate internally only, use zero outside leftmost and rightmost value), and \"none\" (treat NaN as zero everywhere)."
  },
  "pointSize": {
    "default": "1",
    "labels": ["Data Line display"],
  ...
  "labelsSeparateLines": {
    ...
    "labels": ["Legend"],
    "type": "boolean",
    "description": "Put <code>&lt;br/&gt;</code> between lines in the label string. Often used in conjunction wi5,100,200)\" or \"yellow\", etc. If not specified, equally-spaced points around a color wheel are used. Overridden by the 'color' option."
  },
  "connectSeparatedPoints": {
    "default": "false",
    "labels": ["Data Line display"],
  ...
  "labelsKMG2": {
    ...
    "type": "boolean",
    "description": "Show k/M/G for kilo/Mega/Giga on y-axis. This is different than <th <strong>labelsDiv</strong>."
  },
  "xValueFormatter": {
    "default": "",
    "labels": ["Deprecated"],
  ...
  "fractions": {
    ...
    "type": "boolean",
    "description": "When set, attempt to parse each cell in the CSV file as \"a/b\", where a and b are integers. Thecode>labelsKMB</code> in that it uses base 2, not 10."
  },
  "delimiter": {
    "default": ",",
    "labels": ["CSV parsing"],
  ...
  "labelsDivWidth": {
    "default": " ratio will be plotted. This allows computation of Wilson confidence intervals (see below)."
  },
  "logscale": {
    "default": "false",
    "labels": ["Axis display"],
  ...
  "axisLabelFormatter": {
    ...
    "parameters": [
      [ "number or date" , "Either a number (for a 250",
    "labels": ["Legend"],
    "type": "integer",
    "description": "Width (in pixels) of the div which shows information on the currently-highlighted points."
  },
  ...
  "titlenumeric axis) or a Date object (for a date axis)" ],
      [ "granularity" , "specifies how fine-grained the axis is. For date axes, this is a reference to the time granularity enumeration, defined in dygraph-tickers.js, e.g. Dygraph.WEEKLY." ],
      [ "opts" , "a function which provides access to various options on the dygraph, e.g. opts('labelsKMB')." ],
      [ "dygraph" , "the referenced graph" ]
    ],
  ...
  "digitsAfterDecimal" : {
    ...
    "labels": ["Value display/formatting"],
    "type": "integer",
    "description": "Unless it's run in scientific mode (see the <code>sigFigs</code> option), dygraphs displays numbers with <code>digitsAfterDecimal</code> digits after the decimal point. Trailing zeros are not displayed, so with a value of 2 you'll get '0', '0.1', '0.12', '123.45' but not ": {
    "labels": ["Chart labels"],
    "type": "string",
    "default": "null",
    "description": "Text to display above the chart. You can supply any HTML for this value, not just text. If you wish to style it using CSS, use the 'dygraph-label' or 'dygraph-title' classes."
  ...
  "axes": {
    ...
    "type": "Object",
    "description": "Defines per-axis options. Valid keys are 'x', 'y' and 'y2'. Only some options may be set on a per-axis basis. If an option may be set in this way, it will be noted on this page.'123.456' (it will be rounded to '123.46'). Numbers with absolute value less than 0.1^digitsAfterDecimal (i.e. those which would show up as '0.00') will be displayed in scientific notation."
  },
  "maxNumberWidth" : {
    "default": "6",
    "labels": ["Value display/formatting"],
 ...
 * second array entry and an optional extras object for additional data needed. See also documentation on <a href='http://dygraphs.com/per-axis.html'>per-series and per-axis options</a>."
  },
  "series": {
    "default": "null",
    "labels": ["Series"],
  ...
  for (i = 0; i < originalData.length; i++) {
    ...
    if (wilsonInterval) {
      ...
      if (den) {
        ...
        var pm = sigma * Math.sqrt(p * (1 - p) / n + sigma * sigma
  rollPeriod = Math.min(rollPeriod, originalData.length);
  var rollingData = [];
  var sigma = options.get("sigma");
