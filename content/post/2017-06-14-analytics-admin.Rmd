---
title: R for the Enterprise - Analytic Administration for R
author: Nathan Stephens
date: "6/17/2017"
slug: ''
categories: [R, R For The Enterprise]
tags: [R, tidyverse, production, linux]
draft: yes
summary: 'How to multiply your effectiveness by becoming an analytics admin'
---

An analytic admin is a data scientist who can onboard new tools, deploy solutions, support existing standards, and train others on best practices. These people work close with IT to maintain, upgrade, and scale their analytic environments. Analytic admins have a multiplier effect on their work. As they go about their work, they influence others in the organization to be more effective also. If you are a data scientist using R, you might consider filling the role of analytic admin for your organization.
...
![](/home/nathan/projects/rviews/static/post/2017-06-14-analytics-admin/rstudio-toolchain.png)

Most of the software that RStudio makes is open source, but enterprises often require additional professional features. Common pro features are security, authentication, high availability, administration, and load balancing.
 
R is also used with production environments for hosting web applications, exposing API’s, and automating workflows. R is sometimes integrated into other systems such as data warehouses, Hadoop, and Spark. The role of the analytic admin is to provide tooling for data scientists as well as to integrate R into production systems.

# Linux and R

RStudio products run on Linux, so it’s a great tool to learn. Understanding Linux will help you become self sufficient, use R with other systems, and build better solutions. We will talk more about what you can do with Linux commands in an upcoming blog post.
...
Did you know you can execute basic Linux commands from inside RStudio Server using the Tools > Shell menu? You can also execute Linux commands inside the R console with the “system” function.
...
There is a growing trend in using Shiny web apps in production analytic workflows. The vibrant Shiny community now spans all verticals including pharma, high tech, and finance. For many organizations, adopting Shiny is their first experience in running R in production.
 
Production environments that depend on Shiny also need analytic admins who can deploy and support these applications. For example, some organizations now have complex Shiny applications that serve hundreds of end users over a cluster of load balanced Shiny Servers. These applications often go through a standard dev-test-prod deployment process. New tools are being built for [correctness testing](https://github.com/rstudio/shinytest) and [load testing](https://github.com/rstudio/shinyloadtest) in Shiny. RStudio and other platform vendors are making significant investments in building architectures -- like Shiny Server and RStudio Connect -- that will help Shiny grow over the long term.
...
Of course, your installation is going to require more than just installing RStudio Server. You will probably want use the CRAN repository, install Linux dependencies, add users, and manage R packages. Here is a complete script I used to set up RStudio Server on a simple AWS AMI (ami-efd0428f) using a T2-medium instance. I included instructions from [this](https://www.digitalocean.com/community/tutorials/how-to-install-r-on-ubuntu-16-04-2) document on how to install R from CRAN. I also opened port 8787 in my AWS security group so I could log into RStudio Server via my web browser.
...
If you don’t want to install RStudio Server from scratch there are other ways to get started. One is to use a community AMI like [this](http://www.louisaslett.com/RStudio_AMI/) one. Another is to use the [AWS Marketplace](https://aws.amazon.com/marketplace/pp/B06W2G9PRY?qid=1497719355342&sr=0-1&ref_=srh_res_product_title) to install RStudio Server Pro with 1-Click Launch.

# References

Installation is just the first step to administering R. You should also consider authentication, security, scale, integration, hardware sizing, and configuration. Systems administrators have to do a lot of their own training. Analytic admins are no different. Fortunately, there are lots of references to help you get started administering R, RStudio, and Shiny in your organization.
