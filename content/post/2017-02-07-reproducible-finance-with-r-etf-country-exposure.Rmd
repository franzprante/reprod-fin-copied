---
title: 'Reproducible Finance with R: ETF Country Exposure'
author: 'Jonathan Regenstein'
date: '2017-02-07'
categories: [R views]
<<<<<<< HEAD
tags: [Data Science, Finance, R Language, Leaflet]
draft: yes
summary: 'data import/tidy ETF country exposures'
---
...
=======
tags: [finance, leaflet, R]
draft: yes
summary: 'display ETF country exposures'
---
by Jonathan Regenstein

```{r setup, include=FALSE}
lapply(c('dplyr', 'readr', 'tidyr', 'rgdal', 'rmapshaper', 'httr', 'sp', 'leaflet'), function(x) {
...
Before describing our data importing/wrangling work here in the Notebook, it might be helpful to look at where we're headed.  The final Shiny app is [here](http://colorado.rstudio.com:3939/content/277/). This is similar to a [previous project](http://colorado.rstudio.com:3939/content/206/) because we are building a leaflet map, shading it according to data added to the spatial dataframe, and including another HTML widget that is responsive to the map. However, our current project differs in important ways and has a completely different use. 
...
```{r}
>>>>>>> 03184da533aa6be46f2c510c0a18f861729f2f23
library(readr)
library(dplyr)
library(tidyr)
```
...
<<<<<<< HEAD
head(world_fund_country_weights@data[c('name', 'EEM')], n= 20)
