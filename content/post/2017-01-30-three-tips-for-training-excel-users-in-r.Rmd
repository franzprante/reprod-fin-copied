...
[//]: ![](/2017-01-30-three-tips-for-training-excel-users-in-r/excel1.png)
$\qquad$ See? This doesn’t look so complicated.

2. **Show the similarities**. Excel and R do have many similarities in syntax that makes it easier for Excel users to transfer their skills over to R. For example, RStudio has a tabular data view that may look very familiar to spreadsheet users:

[//]: ![](/2017-01-30-three-tips-for-training-excel-users-in-r/excel2.png)
...
\vspace*{\stretch{2}} And this is not just limited to viewing data. There is a lot of syntax from Excel that is easily transferable to R. For example, using if-else statements in Excel looks like this:

```{r fig.width=7, fig.height=2,echo=FALSE}
# http://bit.ly/2kOSaw4
#library(png)
#library(grid)
img <- readPNG("../../static/2017-01-30-three-tips-for-training-excel-users-in-r/excel3.png")
 grid.raster(img, x = unit(.55, "npc"))
```

$\qquad$ And here is what it looks like in R:

[//]: ![](/2017-01-30-three-tips-for-training-excel-users-in-r/excel4.png)

```{r fig.width=7, fig.height=2,echo=FALSE}
# http://bit.ly/2kOSaw4
#library(png)
#library(grid)
img <- readPNG("../../static/2017-01-30-three-tips-for-training-excel-users-in-r/excel4.png")
 grid.raster(img, x = unit(.55, "npc"))
```  
...
#library(png)
#library(grid)
img <- readPNG("../../static/2017-01-30-three-tips-for-training-excel-users-in-r/excel5.png")
 grid.raster(img, x = unit(.55, "npc"))     
```  
