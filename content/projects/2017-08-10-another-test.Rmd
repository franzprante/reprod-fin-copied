...
<iframe src = "https://jkr216.shinyapps.io/Sharpe-Ratio-Flex/" width = "100%"; height =  "700"></iframe>
...
- `roll_portfolio_result` - an `xts` object of rolling portfolio standard deviations. Because both of those are `xts` objects, we can pass them straight to `highcharter` with the `hc_add_series()` function and set a name and a color with the `name` and `color` arguments. Nothing too complicated here - we did the hard work in our previous Notebooks. 
